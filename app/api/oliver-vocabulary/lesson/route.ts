...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const result = generateOliverVocabularyLessonDay(day, defaultEnginePaths(), { persist });
    return apiSuccess({
      action: `Generate Oliver Vocabulary Lesson Day ${day}`,
      ...result,
    });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    const body = (await req.json().catch(() => ({}))) as { day?: unknown; persist?: boolean };
